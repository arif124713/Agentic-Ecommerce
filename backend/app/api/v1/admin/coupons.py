...
async def create_coupon(
    payload: CouponWriteIn, db: AsyncSession = Depends(get_db), _user: User = Depends(require("promo:coupon:write"))
):
    return await AdminCouponService(db).create_coupon(payload)
...
async def update_coupon(
    ...
    return await AdminCouponService(db).update_coupon(coupon_id, payload)
...
async def deactivate_coupon(
    ...
    return await AdminCouponService(db).deactivate_coupon(coupon_id)
